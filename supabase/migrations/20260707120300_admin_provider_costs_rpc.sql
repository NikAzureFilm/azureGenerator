-- admin_provider_costs — per-provider spend aggregated over ALL of
-- provider_usage (not just the newest 20k rows), so month-to-date spend and
-- budget alerts are accurate even in a >20k-row month.
--
-- Read by the admin dashboard's /providers page (RPC-first, with a bounded
-- 20k-row JS aggregation as fallback when this function is absent).
--
-- Canonical dashboard copy: admin/sql/patches/2026-07-07-provider-costs-rpc.sql
-- (byte-identical SQL body). SECURITY DEFINER + service_role-only, guarded with
-- to_regclass so it degrades to an empty set when provider_usage is absent.
