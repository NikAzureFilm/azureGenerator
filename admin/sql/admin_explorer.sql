...
as $$
  with series as (
    select generate_series(
             date_trunc('week', now()) - ((greatest(p_weeks,1) - 1) * interval '1 week'),
             date_trunc('week', now()),
             interval '1 week'
           )::date as week
  )
  select
    s.week,
    coalesce((select count(*) from profiles pr
                where date_trunc('week', pr.created_at)::date = s.week), 0) as signups,
    coalesce((select count(distinct t.user_id) from token_transactions t
                where date_trunc('week', t.created_at)::date = s.week
                  and t.amount < 0 and t.operation <> 'refund'), 0) as active_users,
    coalesce((select count(*) from subscriptions sub
                where date_trunc('week', sub.created_at)::date = s.week), 0) as new_subscriptions
  from series s
  order by s.week;
$$;
...
as $$
  with cohorts as (
    select
      u.id as user_id,
      date_trunc('week', coalesce(pr.created_at, u.created_at))::date as cohort_week
    from auth.users u
    left join profiles pr on pr.user_id = u.id
    where date_trunc('week', coalesce(pr.created_at, u.created_at))
          >= date_trunc('week', now()) - ((greatest(p_weeks,1) - 1) * interval '1 week')
  ),
  sizes as (
  ...
  activity as (
    select
      c.cohort_week,
      floor(
        (date_trunc('week', t.created_at)::date - c.cohort_week) / 7
      )::int as week_offset,
    ...
    where date_trunc('week', t.created_at)::date >= c.cohort_week
    group by c.cohort_week,
             floor((date_trunc('week', t.created_at)::date - c.cohort_week) / 7)::int
  )
  select
    s.cohort_week,
    s.cohort_size,
...
as $$
  select jsonb_build_object(
    ...
    'subscribed',       (select count(distinct user_id) from subscriptions where status in ('active','trialing')),
